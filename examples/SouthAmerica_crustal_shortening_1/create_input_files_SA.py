...
cond = uplift_rate>0
uplift_rate[cond]=np.max(uplift_rate)


uplift_rate2 = np.copy(uplift_rate)
...
#uplift map
np.savetxt("uplift_map.txt",np.c_[uplift_rate,uplift_rate2],fmt="%lg")

plt.figure(figsize=(20,10))
plt.axis("equal")
plt.tricontourf(triang, uplift_rate*1.0E6,levels=30,vmin=0,vmax=2000)
plt.colorbar()
...
plt.figure(figsize=(20,10))
plt.axis("equal")
plt.tricontourf(triang, uplift_rate2*1.0E6,levels=30,vmin=0,vmax=2000)
plt.colorbar()
plt.savefig("Uplift2.png")
plt.close()
